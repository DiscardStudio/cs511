...
do_unregister(State, ClientPID) ->
	case maps:get(ClientPID, State#chat_st.registrations) of
		{badmap, _} ->
			State;
		{badkey, _} ->
			State;
...
do_update_nick(State, ClientPID, NewNick) ->
	case maps:get(ClientPID, State#chat_st.registrations) of
		...
		#chat_st{
			name=State#chat_st.name,
			registrations=maps:put(ClientPID,NewNick,maps:remove(ClientPID,State#chat_st.registrations)),
			history=State#chat_st.history
	}
	end.
...
do_propegate_message(State, Ref, ClientPID, Message) ->
    [ K!{request, self(), Ref, {incoming_msg, V, State#chat_st.name, Message}} || {K,V} <- State#chat_st.registrations ],
	#chat_st {
		name=State#chat_st.name,
