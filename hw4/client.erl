...
loop(State, Request, Ref) ->
	...
	whoami ->
	    {result, self(), Ref, State#cl_st.nick};
...
do_join(State, Ref, ChatName) ->
	case find_item(ChatName,State#cl_st.con_ch) of
		true ->
			whereis(server)!{result, self(), Ref, err};
		false ->
...
do_new_nick(State, Ref, NewNick) ->
    if 
		State#cl_st.nick == NewNick ->
			{{result, self(), Ref, err_same}, State};
		true ->
			whereis(server)!{self(), Ref, nick, NewNick},
			receive
				{_, Ref, err_nick_used} ->
					{{result, self(), Ref, err_nick_used},State};
				{_, Ref, ok_nick} ->
					{response, #cl_st{
						gui=State#cl_st.gui,
						nick=NewNick,
...
do_msg_send(State, Ref, ChatName, Message) ->
    case maps:get(ChatName,State#cl_st.con_ch) of
		{badmap, _} ->
			error;
		{badkey, _} ->
			error;
		Value ->
			Value!{self(), Ref, message, Message},
			{result,State}
	end.
